...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
    ...
    public ResponseEntity<Void> handleDuplicateKeyException(
            DuplicateKeyException ex, NativeWebRequest request) {
        // "Duplicate entry 'value' for key 'keyName'"
        String message = ex.getCause().getMessage();
        String []keyArray = message.split("'");
        if(keyArray.length >= 3){
            message = String.format("{\"key\": \"%s\", \"value\": \"%s\"}", keyArray[3], keyArray[1]);
        }
        return ResponseEntity.badRequest()
                .headers(HeaderUtil.createFailureAlert(message, false,
                        "duplicateKey", ex.getCause().getMessage(), ""))
                .build();
    }
...
//
//    @Override
//    public StackTraceElement[] createStackTrace(Throwable throwable) {
//        return null;
//    }
}
