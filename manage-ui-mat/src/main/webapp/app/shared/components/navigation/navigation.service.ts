...
export {
  CurrentNodes, CurrentNode, NavigationNode, NavigationResponse, NavigationViews, VersionInfo
}from './navigation.model';
...
export class NavigationService {
  /**
   * An observable collection of NavigationNode trees,
   * which can be used to render navigational menus
   */
  ...
  constructor(private http: HttpClient, private location: LocationService) {
    ...
    // The version information is packaged inside the navigation response
   ...
   * We use `publishLast`
   * because once the http request is complete the request observable completes.
   * If you use `publish` here then the completed request observable
   * will cause the subscribed observables to complete too.
   * We `connect` to the published observable to trigger the request immediately.
   * We could use `.refCount` here but then if the subscribers went from 1 -> 0 -> 1
   * then you would get another request to the server.
   * We are not storing the subscription from connecting
   * as we do not expect this service to be destroyed.
   */
  private fetchNavigationInfo(): Observable<NavigationResponse> {
    const navigationInfo = this.http.get<NavigationResponse>('/assets/navigation.json')
      .pipe(publishLast());
  ...
  private getNavigationViews(
    navigationInfo: Observable<NavigationResponse>): Observable<NavigationViews> {
